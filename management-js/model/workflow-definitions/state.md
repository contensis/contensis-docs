...
An identifier for the state, which must be unique within the workflow.
...
Actions to be performed on the entry when it enters the current state.
...
[Ui Editor View](ui-editor-view)

The view used by the Contensis UI to display the entry when it is in this state.

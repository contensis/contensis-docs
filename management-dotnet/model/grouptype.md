1.  [Help and docs](/help-and-docs)
2.  [APIs](/help-and-docs/apis)
3.  [.NET Management API](/help-and-docs/apis/management-dotnet)
4.  Model

[Log in to add to favourites](/account/login)

Page last updated 04 December 2020

The group type enum specifies the types of user groups which Contensis supports.

## Types

Name

Description

Sys

The System type of group, for example for built-in Contensis system groups.

Contensis

A user group created within Contensis.

External

A user group created externally from Contensis, for example Azure AD.

### Still need help?

If you still need help after reading this article, don't hesitate to reach out to the [Contensis community on Slack](https://app.slack.com/client/T5MSMUE3H/C5MSMUYHM) or raise a support ticket to get help from our team.

[New support request](/support)
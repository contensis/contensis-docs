...
Here are some examples of querying this field using our Management API. The main use case for the `searchContent` field is when you are querying multiple content types that could have different text fields that make up the main body copy of an entry, such as in a site-wide search.

...
When searching for a location field, you can also [order the results by distance](https://developer.zengenti.com/contensis/api/delivery/http/search/search-basics.html#ordering-by-distance).

## Sub-queries

A sub-query is a query within another query that is used as a condition to further restrict the results. Effectively they are defined by an explicit nesting of [logical operators](https://developer.zengenti.com/contensis/api/delivery/http/search/query-operators.html#logical-operators).
...
When [searching by location](https://developer.zengenti.com/contensis/api/delivery/http/search/search-basics.html#location-searches), to return the search results according to the distance of the location field in each entry from the distance specified in the distanceWithin search, add the location field to the [orderBy](https://developer.zengenti.com/contensis/api/delivery/http/search/search-basics.html#ordering) clause, e.g.
